...
import git  # Added import for Git operations
from subprocess import run  # Added import for running subprocesses
...
# Функция для обновления кода из репозитория
def update_code():
    try:
        repo = git.Repo('.')
        origin = repo.remotes['origin']  # Ensure we're using the 'origin' remote
        repo_url = origin.url
        expected_url = "https://github.com/prem1umservice/brave3690.git"  # Replace with your actual repo URL

        if repo_url != expected_url:
            print(f"Ошибка: текущий репозиторий ({repo_url}) не совпадает с ожидаемым ({expected_url}).")
            return

        origin.pull()
        print("Код успешно обновлён из репозитория.")
        # После обновления можно запустить новый код
        run(['python', 'main.py'])
    except Exception as e:
...
if __name__ == "__main__":
    update_code()  # Call the update_code function before running the main logic
    main()
